...
class StandardGA:
    ...
    def run(self, max_generation):
        ...
        for generation_num in range(max_generation):
            ...
            if self.elitism:
                # The new computed population has length n-1
            ...
            worst_individual.append(self.population[0].fitness_val)
            best_individual.append(self.population[-1].fitness_val)
            self.population = next_population
            self._sort_population()
            self._update_solution(self.population[-1].chromosome, self.population[-1].fitness_val)
            
            print('Geração', generation_num + 1, 'terminada')
            time.sleep(10)
        
        best_individual.append(self.population[-1].fitness_val)
        fitness_progress.append(fitness_sum / population_size)
        return fitness_progress, best_individual, worst_individual
