...
class StandardGA(object):
    ...
    def _random_diff(self, stop, n, start=0):
        ...
        if stop - start < n:
            ...
            raise ValueError(u'There is not enough numbers in the given interval.')
        elif stop - start == n:
    ...
    def _invert_bit(self, chromosome, bit_num):
        ...
        Returns:
            mutated chromosome
        """
        raise NotImplementedError(u'This method must be reimplemented in subclasses.')
    ...
    def _mutate(self, chromosome):
        ...
        if self._bin_length == self.mut_type:
            ...
            chromosome = self._invert_bit(chromosome, xrange(self._bin_length))
        else:
    ...
    def _conduct_tournament(self, population, size):
        ...
        if size < 1 or population is None:
            raise ValueError(u'Wrong input parameter.')

        try:
            population_size = len(population)
        except TypeError:
            raise ValueError(u'Population must be a list.')

        if population_size < 1:
            raise ValueError(u'Too small population.')
    ...
    def _select_parents(self, population, wheel_sum=None):
        ...
        if self.selection in [u'roulette', u'rank']:
            if wheel_sum is None or wheel_sum <= 0:
                ...
                raise ValueError(u'Wrong value of wheel sum')
            ...
            raise ValueError(u'Unknown selection type')
    ...
    def _update_solution(self, chromosome, fitness_val):
        ...
        Args:
            ...
            fitness_val (float, int): Fitness value of the given chromosome.
        """
        if (self.optim == u'min' and fitness_val < self.best_fitness)\
                or (self.optim == u'max' and fitness_val > self.best_fitness):
    ...
    def init_population(self, chromosomes, interval=None):
        ...
        if not chromosomes or len(chromosomes) < 4:
            raise ValueError(u'New population is too small.')

        if not hasattr(self, u'_data'):
            if interval is None or interval[0] >= interval[1]:
                raise ValueError(u'You must specify a correct interval for RealGA.')
    ...
    def run(self, max_generation):
        ...
        """
        if max_generation < 1:
            raise ValueError(u'Too few generations...')
